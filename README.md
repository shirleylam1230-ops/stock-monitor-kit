...
一个面向个人投资者的**自动化监控与日报系统**：持仓/观察池 MACD 技术面分析、宏观微观指标监测、FOMC 与财经日历、关注博主（抖音/小红书/微博/雪球）T-1 更新追踪，产出一个多屏网页 App + 每日邮件推送。
...
| 博主动态 | T-1 窗口更新检测、改名检测、抖音视频 AI 章节要点总结 | CDP 真实 Chrome |
| 产出 | 5 屏网页 App（首页/持仓/宏观/博主/归档）+ 邮件摘要 | 静态 HTML + SMTP/邮件机器人 |
...
│   ├── fetch_social.py        # 四平台博主抓取 → data/social_YYYYMMDD.json
│   ├── build_daily_report.py  # 日报 HTML + 邮件摘要 + 归档页
│   ├── build_holdings_report.py # 持仓专题报告
│   └── build_app.py           # 网页 App 五屏生成
...
3. **配置脚本**：`data/creators.json` 填自己的博主；`data/positions.json` 由云端表自动镜像
...
6. **建两个定时任务**（工作日）：
   ...
   - 19:00 盘后版：A 股收盘复盘 + 博主 T-1 + 技术状态变化
   完整任务提示词见下文「自动化任务模板」。
...
4. fetch_social.py --date <D> --max-items 3    # 需登录态，约 8 分钟
...
- `data/` 全部个人数据不进 git；脚本目录不含任何个人信息
