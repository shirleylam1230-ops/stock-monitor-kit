...
把「对话版」的日报能力产品化成常驻站点。当前覆盖两屏：
  reports/index.html     首页总览（市场速览 + 持仓 KPI + MACD 分布 + 事件 + 博主）
...
数据来源（全部读本地文件，不联网）：
  ...
  data/social_YYYYMMDD.json    博主 T-1 更新
...
# AI 算力链成分（用于集中度口径，改动请同步说明文字）
AI_CHAIN = {"sz300308": "中际旭创", "sz300502": "新易盛", "sh601869": "长飞光纤"}
...
<a class="tab{t3}" href="social.html">博主动态</a>
<a class="tab{t4}" href="archive.html">日报归档</a>
</div></nav>"""


def shell(active, title, body, asof, extra_js=""):
    tabs = {i: "" for i in range(5)}
    tabs[active] = " on"
    nav = NAV.format(t0=tabs[0], t1=tabs[1], t2=tabs[2], t3=tabs[3], t4=tabs[4])
...
def render_index(pos, tech, daily, social, rows, total_mv, sumr, asof, fomc):
    ...
    # 博主
    creators = (social.get("creators") or [])
    if creators:
        hits = [(c, c.get("posts_window") or []) for c in creators]
        hits = [(c, p) for c, p in hits if p]
        out.append(f"<h2>关注博主 T-1 更新（{len(hits)}/{len(creators)} 位有更新）</h2>")
        out.append("<div class='card' style='padding:14px 16px'>")
        CN = {"douyin": "抖音", "xhs": "小红书", "weibo": "微博", "xueqiu": "雪球", "wechat": "公众号"}
        if not hits:
            out.append("<div class='miss'>统计窗口内所有博主均无更新。</div>")
        for c, posts in sorted(hits, key=lambda x: CN.get(x[0].get("platform"), "")):
            out.append(f"<div style='margin:7px 0'>"
                       f"<span class='pill' style='background:#EEF2F7;color:#4B5563'>"
                       f"{esc(CN.get(c.get('platform'), c.get('platform')))}</span> "
                       f"<b style='font-size:13px'>{esc(c.get('nickname'))}</b></div>")
            for p in posts[:2]:
                out.append(f"<div style='font-size:12px;color:#4B5563;padding-left:4px'>"
                           f"· {esc(p.get('published'))}　{esc((p.get('title') or '')[:78])}</div>")
        out.append("</div>")
    ...
    for href, t, s in (("holdings.html", "持仓分析", "一票一卡 · 可排序筛选"),
                       ...
                       ("social.html", "博主动态", "四平台 · T-1 更新与最近发布"),
...
{('<div class="tip" style="color:' + WARN + '">备注：' + esc(r['note']) + '</div>') if r.get('note') else ''}
    ...
    if wrows:
        ...
        out.append("<table><thead><tr><th>标的</th><th>类型</th><th>MACD 状态</th>"
...
# ---------------------------------------------------------------- 博主动态
SOCIAL_CN = {"douyin": "抖音", "xhs": "小红书", "weibo": "微博", "xueqiu": "雪球", "wechat": "公众号"}
SOCIAL_ORDER = {"xhs": 0, "douyin": 1, "weibo": 2, "xueqiu": 3}


def render_social(social):
    creators = sorted(social.get("creators") or [],
                      key=lambda c: (SOCIAL_ORDER.get(c.get("platform"), 9),
                                     (c.get("nickname") or "")))
    # 昵称兜底：抓取偶发拿不到 title 时，从 creators_state.json 的历史映射回填
    state = load_json(os.path.join(DATA, "creators_state.json"), {}) or {}
    for c in creators:
        if not c.get("nickname"):
            c["nickname"] = ((state.get(f"{c.get('platform')}:{c.get('platform_uid')}") or {})
                             .get("nickname")) or c.get("platform_uid")
    win = social.get("window") or []
    hits = [c for c in creators if c.get("posts_window")]
    platforms = sorted({c.get("platform") for c in creators},
                       key=lambda p: SOCIAL_ORDER.get(p, 9))
    out = []
    out.append("<h2>监测概览</h2><div class='grid g4'>")
    out.append(f"<div class='tile'><div class='k'>监测博主</div><div class='v'>{len(creators)}</div>"
               f"<div class='s'>{'、'.join(SOCIAL_CN.get(p, p) for p in platforms)}</div></div>")
    out.append(f"<div class='tile'><div class='k'>T-1 窗口有更新</div><div class='v' style='color:{RED}'>"
               f"{len(hits)}</div><div class='s'>窗口：{'、'.join(win) if win else '—'}</div></div>")
    err = [c for c in creators if c.get("status") not in ("ok",)]
    out.append(f"<div class='tile'><div class='k'>异常账号</div>"
               f"<div class='v' style='color:{WARN if err else INK}'>{len(err)}</div>"
               f"<div class='s'>{'、'.join(c.get('nickname') or c.get('platform_uid') or '' for c in err) or '全部正常'}</div></div>")
    out.append(f"<div class='tile'><div class='k'>数据日期</div>"
               f"<div class='v' style='font-size:15px'>{esc(social.get('date') or '—')}</div>"
               f"<div class='s'>每日 08:00 / 19:00 自动刷新</div></div>")
    out.append("</div>")

    # 平台筛选
    out.append("""<h2>博主卡片</h2><div class="tools"><span class="lb">平台</span>
<button class="btn on" data-pf="all">全部</button>""")
    for p in platforms:
        out.append(f"<button class='btn' data-pf='{esc(p)}'>{SOCIAL_CN.get(p, p)}</button>")
    out.append("</div><div class='cards' id='scards'>")

    for c in creators:
        pf = c.get("platform")
        cn = SOCIAL_CN.get(pf, pf)
        status = c.get("status")
        wposts = c.get("posts_window") or []
        ap_all = c.get("posts_all") or []
        if status != "ok":
            badge = f"<span class='pill' style='background:#FFFBEB;color:#B45309'>{esc(status)}</span>"
        elif wposts:
            badge = f"<span class='pill' style='background:#FEF2F2;color:{RED}'>T-1 有更新 {len(wposts)} 条</span>"
        else:
            badge = "<span class='pill' style='background:#F1F3F6;color:#6B7280'>窗口内无更新</span>"
        border = f"border-left:3px solid {RED};" if wposts else ""
        head = (f"<a href='{esc(c.get('home_url') or '#')}' target='_blank' rel='noopener'>"
                f"<span class='nm'>{esc(c.get('nickname') or c.get('platform_uid'))}</span></a> "
                f"<span class='pill' style='background:#EEF2F7;color:#4B5563'>{esc(cn)}</span>")
        body = []
        if wposts:
            body.append("<div style='font-size:11.5px;color:#C62828;margin:8px 0 2px;font-weight:600'>"
                        "窗口内更新</div>")
            for p in wposts:
                body.append(f"<div style='font-size:12px;padding:3px 0'>"
                            f"<span style='color:#9AA1AC'>{esc(p.get('published'))}</span>　"
                            f"<a href='{esc(p.get('url') or '#')}' target='_blank' rel='noopener' "
                            f"style='color:#1F2933'>{esc((p.get('title') or '')[:96])}</a></div>")
                summ = (p.get("summary") or "").strip()
                if summ:
                    body.append(f"<div style='font-size:11.5px;color:#57606A;background:#F7F8FA;"
                                f"border-left:2px solid #D8DDE4;padding:5px 8px;margin:2px 0 4px;"
                                f"border-radius:4px;line-height:1.55'>"
                                f"{esc(summ[:180])}{'…' if len(summ) > 180 else ''}</div>")
        body.append("<div style='font-size:11.5px;color:#6B7280;margin:9px 0 2px'>最近发布</div>")
        for p in ap_all[:5]:
            dup = any(p.get("url") == w.get("url") for w in wposts)
            if dup:
                continue
            body.append(f"<div style='font-size:12px;color:#4B5563;padding:2.5px 0'>"
                        f"<span style='color:#9AA1AC'>{esc(p.get('published'))}</span>　"
                        f"<a href='{esc(p.get('url') or '#')}' target='_blank' rel='noopener' "
                        f"style='color:#4B5563'>{esc((p.get('title') or '')[:88])}</a></div>")
        if not ap_all and not wposts:
            body.append(f"<div class='miss' style='margin-top:6px'>{esc(c.get('note') or '本次未取到内容')}</div>")
        if c.get("note") and (ap_all or wposts):
            body.append(f"<div class='hint'>{esc(c['note'])}</div>")
        out.append(f"<div class='hcard' data-pf='{esc(pf)}' style='{border}'>"
                   f"<div class='top'><div>{head}<div class='cd'>{esc(c.get('platform_uid'))}</div></div>"
                   f"{badge}</div><div style='margin-top:2px'>{''.join(body)}</div></div>")
    out.append("</div>")
    if not creators:
        out.append("<div class='miss'>社媒数据缺失，请先运行 fetch_social.py</div>")

    js = """<script>
(function(){
  var cards=[].slice.call(document.querySelectorAll('#scards .hcard'));
  document.querySelectorAll('.tools .btn[data-pf]').forEach(function(b){
    b.addEventListener('click',function(){
      document.querySelectorAll('.tools .btn[data-pf]').forEach(function(x){x.classList.remove('on')});
      b.classList.add('on');
      var f=b.dataset.pf;
      cards.forEach(function(c){
        c.style.display=(f==='all'||c.dataset.pf===f)?'':'none';
      });
    });
  });
})();
...
def main():
    ...
    pos, tech, daily, social, fomc, macro = load_inputs()
    ...
    with open(p1, "w", encoding="utf-8") as fh:
        fh.write(shell(0, "投资 App · 首页总览",
                       render_index(pos, tech, daily, social, rows, total_mv, sumr, asof, fomc), asof))
    print("written:", p1)
    ...
    sbody, sjs = render_social(social)
    p4 = os.path.join(REPORTS, "social.html")
    with open(p4, "w", encoding="utf-8") as fh:
        fh.write(shell(3, "投资 App · 博主动态", sbody, asof, extra_js=sjs))
    print("written:", p4)
