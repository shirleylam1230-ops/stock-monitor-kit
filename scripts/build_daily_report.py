...
IMPACT_COLOR = {"利好": "#C62828", "利空": "#2E7D32", "中性": "#6B7280"}


def load_ai_digest(run_date):
    """读取当日 AI 产业链动态摘要（data/ai_digest_<stamp>.json，由日报流程检索生成）。"""
    p = os.path.join(ROOT, "data", f"ai_digest_{run_date.replace('-', '')}.json")
...
def sec_read(d, run_date):
    """当日市场解读：规则化结论（阈值见 market_read.py）+ AI 产业链动态，缺数据明确标注。"""
    from html import escape as esc
    blocks = MR.market_read(d)
    out = ["<h2>七、当日市场解读</h2>",
           "<div class='meta'>基于固定阈值的规则化判读（阈值见 scripts/market_read.py），"
           "市场画像来自 westock 量化打分。仅供参考，不构成投资建议。</div>"]
    for b in blocks:
        c = MR.LEVEL_COLOR.get(b["level"], MR.GRAY)
        out.append(f"<div class='box' style='margin:8px 0'>"
                   f"<div style='display:flex;justify-content:space-between;align-items:baseline'>"
                   f"<b style='color:{INK};font-size:13.5px'>{esc(b['tag'])}</b>"
                   f"<span style='background:{c}1A;color:{c};font-size:12px;font-weight:600;"
                   f"padding:2px 10px;border-radius:99px'>{esc(b['verdict'])}</span></div>"
                   f"<div style='font-size:12.8px;color:#4B5563;margin-top:6px;line-height:1.6'>"
                   f"{esc(b['detail'])}</div></div>")

    # AI 产业链动态：持仓 71% 集中在 AI 算力链，黄仁勋发言 / OpenAI·Anthropic 商业化 /
    # 华为芯片突破 / 云厂商资本开支都会直接冲击持仓估值
    dg = load_ai_digest(run_date)
    out.append("<h3>AI 产业链动态</h3>")
    if not dg or not (dg.get("items") or []):
        out.append("<div class='miss'>AI 产业链动态：本次未取到。</div>")
    else:
        for it in (dg.get("items") or []):
            c = IMPACT_COLOR.get(it.get("impact"), "#6B7280")
            out.append(f"<div class='box' style='margin:8px 0'>"
                       f"<div style='display:flex;justify-content:space-between;align-items:baseline'>"
                       f"<b style='color:{INK};font-size:13px'>{esc(it.get('title') or '')}</b>"
                       f"<span style='background:{c}1A;color:{c};font-size:11.5px;font-weight:600;"
                       f"padding:2px 9px;border-radius:99px;white-space:nowrap'>{esc(it.get('impact') or '中性')}</span></div>"
                       f"<div style='font-size:12px;color:#9AA1AC;margin-top:3px'>{esc(it.get('time') or '')}　{esc(it.get('source') or '')}</div>"
                       + (f"<div style='font-size:12.8px;color:#4B5563;margin-top:5px;line-height:1.6'>{esc(it['note'])}</div>"
                          if it.get("note") else "")
                       + "</div>")
    return "".join(out)
...
def build(d, tech, run_date, mode):
    ...
    html.append(sec_read(d, run_date))
...
def main():
    ...
    if os.path.exists(tech_path):
        with open(tech_path, encoding="utf-8") as fh:
            tech = json.load(fh)
    # 社媒模块已于 2026-09-16 下线：不再读取 social_*.json，日报与摘要均无博主板块
    if args.summary:
    ...
    with open(out, "w", encoding="utf-8") as fh:
        fh.write(build(d, tech, args.date, args.mode))
    print("written:", out)
    index = build_index()
    print("written:", index)
...
def email_summary(d, tech, run_date, mode, base_url=""):
    ...
    # 当日市场解读（规则化结论，阈值见 scripts/market_read.py）
    read = MR.read_lines(d)
    if read:
        lines.append("")
        lines.append("**当日市场解读**：")
        lines.extend(f"- {x}" for x in read)

    # AI 产业链动态（重仓均为 AI 硬件，行业事件直接冲击持仓估值）
    dg = load_ai_digest(run_date)
    lines.append("")
    if dg and (dg.get("items") or []):
        lines.append("**AI 产业链动态**：")
        for it in dg["items"][:6]:
            lines.append(f"- [{it.get('impact') or '中性'}] {it.get('title') or ''}（{it.get('source') or ''}）")
            if it.get("note"):
                lines.append(f"  {it['note']}")
    else:
        lines.append("**AI 产业链动态**：本次未取到。")
        lines.append("")
    ...
    # 关注博主模块已于 2026-09-16 下线，摘要不再包含博主内容
...
def build_index():
    ...
    # CDN 缓存治理：同 build_app.py，给站内链接追加 ?v=<发布时刻>，避免 CDN 按旧 URL 缓存
    import re
    v = datetime.now().strftime("%m%d%H%M")
    html = re.sub(r"(href=[\"'])((?:index|holdings|macro|archive|daily-\d+|holdings-report-\d+)\.html)",
                  rf"\1\2?v={v}", html)
    with open(path, "w", encoding="utf-8") as fh:
        fh.write(html)
    return path
