...
# A 股 AI 链代表标的：从本地配置读取（config/targets.json，git 忽略），
# 代码内不写个人标的清单。
AI_A = targets.ai_a()
...
# 两市成交额（沪市总成交 + 深市总成交，注意深证成指只含成份股，不能用）
TURNOVER_IDX = [("sh000001", "沪市"), ("sz399106", "深市")]
...
def collect(asof):
    ...
    # ---------------- 两市成交额（沪市 + 深市总量）
    def turnover():
        # 注意：quote_rows 会把字段归一化掉 amount，这里直接解析原始行情表
        out = run(f"westock quote {','.join(c for c, _ in TURNOVER_IDX)}")
        rows = {r.get("code"): r for r in parse_md_table(out) if r.get("code")}
        parts, total = {}, 0.0
        for c, label in TURNOVER_IDX:
            amt = f(rows.get(c) or {}, "amount")
            if amt:
                parts[label] = amt
                total += amt
        if total <= 0:
            return None
        return dict(date=asof,
                    **{f"{k}_yi": round(v / 1e8, 0) for k, v in parts.items()},
                    total_yi=round(total / 1e8, 0))
    d["turnover"] = safe("两市成交额", turnover)

    # ---------------- A 股市场总览画像（westock 量化打分：情绪/宽度/量能/估值等）
    def market_overview():
        out = run("westock market-overview")
        dims = []
        mdate = None
        for line in out.splitlines():
            m = re.match(r"`?数据日期\s*`?(\d{4}-\d{2}-\d{2})", line)
            if m:
                mdate = m.group(1)
            m = re.match(r"\|\s*([^|]+?)\s*\|\s*(\d+)\s*\|\s*(.+?)\s*\|\s*$", line)
            if m and m.group(1) not in ("维度", "---"):
                dims.append(dict(dim=m.group(1), score=int(m.group(2)), status=m.group(3)))
        score = re.search(r"原始评分\s*\*?\*?\s*(\d+\.?\d*)", out)
        adj = re.search(r"调整评分\s*\*?\*?\s*(\d+\.?\d*)", out)
        if not dims:
            return None
        return dict(date=mdate, score_raw=f({"v": score.group(1)}, "v") if score else None,
                    score_adj=f({"v": adj.group(1)}, "v") if adj else None, dims=dims)
    d["market_overview"] = safe("市场总览画像", market_overview)
